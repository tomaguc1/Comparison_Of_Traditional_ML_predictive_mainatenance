...
# hyperPar_x = [np.arange(5, 60, 15), 'Minimum_Sample_Split']
# hyperPar_y = [np.arange(2, 40, 15), 'No_Estimators']


hyperPar_x = [np.arange(2, 15, 2), 'Minimum_Sample_Split']
hyperPar_y = [np.arange(15, 30, 3), 'No_Estimators']
...
for y in tqdm(range(len(hyperPar_y[0]))):
    ...
    for i, x in enumerate(hyperPar_x[0]):
        ...
        model = RandomForestClassifier(n_estimators= (hyperPar_y[0])[y],
...
np.save('RandForest_Errors_' + '_'.join([hyperPar_x[1], hyperPar_y[1]]) 
            + '.npy', Val_error_array)
...
plt.title(f'RandForest Min error param : {hyperPar_x[1]} = {(hyperPar_x[0])[min_error_idx[1]]} | {hyperPar_y[1]} = {(hyperPar_y[0])[min_error_idx[0]]}')
