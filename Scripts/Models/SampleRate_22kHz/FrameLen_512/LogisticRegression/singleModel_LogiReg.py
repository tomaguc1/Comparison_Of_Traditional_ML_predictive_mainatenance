...
title(f'LogiReg X_val predictions : Error = {str( round( misclass_rate_val, 4) )}')
legend()
...
plot(fpr[optimal_idx], tpr[optimal_idx], 'o', markersize= 8, markerfacecolor= 'none', markeredgecolor= 'firebrick', markeredgewidth=2, label= f'Thresh = {round(optimal_threshold_prob, 2)} TPR = {round(tpr[optimal_idx], 2)}' )
