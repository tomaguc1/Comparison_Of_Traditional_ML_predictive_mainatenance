...
model = tree.DecisionTreeClassifier(criterion= criterion, random_state= 25) 
# model = tree.DecisionTreeClassifier(criterion= criterion, max_depth= 28, min_samples_split=6, random_state= 25) 
...
plot(classBoth_misclass_ids, y_est_train_prob[classBoth_misclass_ids], 'o', markersize= 8, markeredgecolor='y', markerfacecolor='none', markeredgewidth=1, label= 'Misclassified observations')
    # |
    # |  Threshold
...
_mask_threshold_ids = np.linspace(min(classBoth_ids), max(classBoth_ids), 25)  ;len_mask_thresh = len(_mask_threshold_ids) 
plot(_mask_threshold_ids, [0.5]*len_mask_thresh, ':', color= '#088da5',linewidth= 2 ,label= 'Threshold 0.5')
plot(_mask_threshold_ids, [optimal_threshold_prob]*len_mask_thresh, '--', color= '#f6546a' , linewidth=2, label= f'ROC optimal threshold = {str("{:.4f}".format(optimal_threshold_prob))}')
...
plot(fpr, tpr, color='blue', label=f'ROC_line (AUC = {AUC_val:.4f})')
plot(fpr[optimal_idx], tpr[optimal_idx], 'o', markersize= 8, markerfacecolor= 'none', markeredgecolor= 'firebrick', markeredgewidth=2, label= f'Thresh = {round(optimal_threshold_prob, 4)} TPR = {round(tpr[optimal_idx], 2)}' )
