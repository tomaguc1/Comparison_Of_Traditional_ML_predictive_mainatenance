...
# model = lm.LogisticRegression(random_state= 25, max_iter= 99999)
model = lm.LogisticRegression(C=1/0.0001, random_state= 25, max_iter= 99999)#C=1/0.01
...
plot(fpr, tpr, color='blue', label=f'ROC_line (AUC = {round(AUC_val, 4)})')
plot(fpr[optimal_idx], tpr[optimal_idx], 'o', markersize= 8, markerfacecolor= 'none', markeredgecolor= 'firebrick', markeredgewidth=2, label= f'Thresh = {round(optimal_threshold_prob, 4)} TPR = {round(tpr[optimal_idx], 4)}')
